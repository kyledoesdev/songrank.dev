        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div class="whitespace-nowrap">
                ...
                <p class="text-xs text-zinc-500 mt-0.5">
                    {{ number_format($totalTierlists) }} completed {{ Str::plural('tier list', $totalTierlists) }} to explore
                </p>
            </div>
            ...
            <div wire:intersect="loadMore" class="mt-4">
                <div class="animate-pulse grid grid-cols-1 lg:grid-cols-2 gap-4">
                    @foreach (range(1, 4) as $_)
                        <x-tierlists.card-placeholder wire:transition />
                    @endforeach
                </div>
            </div>
        @endif
